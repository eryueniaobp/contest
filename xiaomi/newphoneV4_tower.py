...
from TowerRankingModel import NewPhoneModel
def build_functional_complied_model_with_towers()  -> tfrs.models.Model :
    model = NewPhoneModel()
...
if __name__ == '__main__':
    ...
    trainparser.add_argument('--model', choices=['lstm', 'cnn', 'moe', 'tt'], required=True)

    testparser = subparser.add_parser('test')
    testparser.add_argument('--model', choices=['lstm', 'cnn', 'moe', 'tt'], required=True)
    ...
    if args.task == 'train':
        if args.model == 'lstm':
            ...
            model = build_functional_complied_model_with_towers()
            model_path = './tt_model/weights'
            csvfile = './tt.csv'
        ...
        if args.model == 'tt':
            # input(" no weight here!")
            # for x, _ in dataset.take(100):
            #     print(x['resident_city_type'])
            #
            # import sys
            #
            # sys.exit(1)
